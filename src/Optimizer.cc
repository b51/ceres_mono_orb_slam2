 ...
 *
 *          Created On: Thu 05 Sep 2019 11:07:37 AM CST
...
void Optimizer::GlobalBundleAdjustemnt(Map* map, int nIterations,
                                       bool* pbStopFlag,
                                       const unsigned long nLoopKF,
                                       const bool bRobust) {
  vector<KeyFrame*> keyframes = map->GetAllKeyFrames();
  vector<MapPoint*> map_points = map->GetAllMapPoints();
  BundleAdjustment(keyframes, map_points, nIterations, pbStopFlag, nLoopKF,
                   bRobust);
}

void Optimizer::BundleAdjustment(const vector<KeyFrame*>& keyframes,
                                 const vector<MapPoint*>& map_points,
                                 int nIterations, bool* pbStopFlag,
                                 const unsigned long nLoopKF,
                                 const bool bRobust) {
  vector<bool> vbNotIncludedMP;
  vbNotIncludedMP.resize(map_points.size());
  ...
  for (size_t i = 0; i < keyframes.size(); i++) {
    ...
    if (keyframe->id_ > max_keyframe_id) max_keyframe_id = keyframe->id_;
  }
  ...
  for (size_t i = 0; i < map_points.size(); i++) {
    ...
    for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin();
      ...
      if (keyframe->isBad() || keyframe->id_ > max_keyframe_id) continue;

      nEdges++;

      const cv::KeyPoint& kpUn = keyframe->undistort_keypoints_[mit->second];
      ...
      e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                          optimizer.vertex(keyframe->id_)));
      e->setMeasurement(obs);
      const float& invSigma2 = keyframe->inv_level_sigma2s_[kpUn.octave];
  ...
  for (size_t i = 0; i < keyframes.size(); i++) {
    ...
    g2o::VertexSE3Expmap* vSE3 =
        static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(keyframe->id_));
    g2o::SE3Quat SE3quat = vSE3->estimate();
    if (nLoopKF == 0) {
      keyframe->SetPose(Converter::toCvMat(SE3quat));
    } else {
      keyframe->global_BA_Tcw_.create(4, 4, CV_32F);
      Converter::toCvMat(SE3quat).copyTo(keyframe->global_BA_Tcw_);
      keyframe->n_BA_global_for_keyframe_ = nLoopKF;
    }
  }
  ...
  for (size_t i = 0; i < map_points.size(); i++) {
    ...
    g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(
        optimizer.vertex(map_point->id_ + max_keyframe_id + 1));
    ...
    if (nLoopKF == 0) {
      ...
      Converter::toCvMat(vPoint->estimate()).copyTo(map_point->global_BA_pose_);
      map_point->n_BA_global_for_keyframe_ = nLoopKF;
    }
  }
}

int Optimizer::NoneInlierPoseOptimization(Frame* pFrame) {
  g2o::SparseOptimizer optimizer;
  g2o::BlockSolver_6_3::LinearSolverType* linearSolver;
...

  {
    ...
    for (int i = 0; i < N; i++) {
      MapPoint* map_point = pFrame->map_points_[i];
      if (map_point) {
        // Monocular observation
        if (pFrame->mvuRight[i] < 0) {
          nInitialCorrespondences++;
          pFrame->is_outliers_[i] = false;

          Eigen::Matrix<double, 2, 1> obs;
          const cv::KeyPoint& kpUn = pFrame->undistort_keypoints_[i];
          obs << kpUn.pt.x, kpUn.pt.y;
          ...
          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(0)));
          e->setMeasurement(obs);
          const float invSigma2 = pFrame->inv_level_sigma2s_[kpUn.octave];
          e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
...
int Optimizer::PoseOptimization(Frame* pFrame) {
  ...
  for (size_t it = 0; it < 4; it++) {
    ...
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
      ...
      if (chi2 > chi2Mono[it]) {
        pFrame->is_outliers_[idx] = true;
        e->setLevel(1);
        nBad++;
      } else {
        pFrame->is_outliers_[idx] = false;
        e->setLevel(0);
      }
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag,
  ...
  pKF->n_BA_local_for_keyframe_ = pKF->id_;

  const vector<KeyFrame*> vNeighKFs = pKF->GetVectorCovisibleKeyFrames();
  for (int i = 0, iend = vNeighKFs.size(); i < iend; i++) {
    KeyFrame* pKFi = vNeighKFs[i];
    pKFi->n_BA_local_for_keyframe_ = pKF->id_;
  ...
  for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    for (vector<MapPoint*>::iterator vit = vpMPs.begin(), vend = vpMPs.end();
      ...
      if (map_point)
        if (!map_point->isBad())
          if (map_point->n_BA_local_for_keyframe_ != pKF->id_) {
            lLocalMapPoints.push_back(map_point);
            map_point->n_BA_local_for_keyframe_ = pKF->id_;
          }
    }
  }
  ...
  for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame*, size_t>::iterator mit = observations.begin(),
      ...
      if (pKFi->n_BA_local_for_keyframe_ != pKF->id_ &&
          pKFi->n_BA_fixed_for_keyframe_ != pKF->id_) {
        pKFi->n_BA_fixed_for_keyframe_ = pKF->id_;
  ...
  for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    optimizer.addVertex(vSE3);
    if (pKFi->id_ > max_keyframe_id) max_keyframe_id = pKFi->id_;
  }
  ...
  for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin(),
      ...
      if (!pKFi->isBad()) {
        const cv::KeyPoint& kpUn = pKFi->undistort_keypoints_[mit->second];
        ...
        if (pKFi->mvuRight[mit->second] < 0) {
          ...
          e->setMeasurement(obs);
          const float& invSigma2 = pKFi->inv_level_sigma2s_[kpUn.octave];
          e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
  ...
  for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
    ...
    if (e->chi2() > 5.991 || !e->isDepthPositive()) {
      ...
      vToErase.push_back(make_pair(pKFi, map_point));
    }
  }
  ...
  // Get Map Mutex
  unique_lock<mutex> lock(pMap->mutex_map_update_);
...
void Optimizer::OptimizeEssentialGraph(
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
    MapPoint* map_point = vpMPs[i];

    if (map_point->isBad()) continue;

    int nIDr;
    if (map_point->corrected_by_keyframe_ == pCurKF->id_) {
      nIDr = map_point->corrected_reference_;
    } else {
      KeyFrame* pRefKF = map_point->GetReferenceKeyFrame();
      nIDr = pRefKF->id_;
    }
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
  ...
  // Calibration
  const cv::Mat& K1 = pKF1->K_;
  const cv::Mat& K2 = pKF2->K_;
  ...
  for (int i = 0; i < N; i++) {
    ...
    const cv::KeyPoint& kpUn1 = pKF1->undistort_keypoints_[i];
    ...
    const float& invSigmaSquare1 = pKF1->inv_level_sigma2s_[kpUn1.octave];
    ...
    const cv::KeyPoint& kpUn2 = pKF2->undistort_keypoints_[i2];
    ...
    float invSigmaSquare2 = pKF2->inv_level_sigma2s_[kpUn2.octave];
